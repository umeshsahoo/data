Rem
Rem $Header: a1001000.sql 21-jun-2005.16:48:21 nbhatt Exp $
Rem
Rem a1001000.sql
Rem
...
Rem    nbhatt      06/21/05 - upgrade must handle queue tables with errors 
Rem    dsampath    06/22/05 - 4449955 fix upgrade for sqltext signature 
...
BEGIN
  ...
  LOOP  
    ...
    new_sig := sys.dbms_sqltune.sqltext_to_signature(sqltext, 
                                                     sig_rec.force_flag);  
    ...
    EXCEPTION
     WHEN DUP_VAL_ON_INDEX THEN
       ...
       sys.dbms_sqltune_internal.test_ksd_trace(
...
Rem
Rem Changes in advisor definitions: make sqltune task resumable 
Rem
...
DECLARE
  ...
  -- For iterating over the statements table, to compute the force match
  -- signature
  CURSOR stmt_cur IS
    SELECT s.sql_id, st.sql_text
    FROM   wri$_sqlset_statements s, wrh$_sqltext st
...
BEGIN
  ...
  IF (already_r2 = 0) THEN
    ...
    EXECUTE IMMEDIATE
      ...
      'SELECT wri$_sqlset_stmt_id_seq.NEXTVAL id, s.sqlset_id, s.sql_id, 0, ' ||
    ...
    -- Compute the force matching signature (we must loop because of the
    -- non-sql boolean type)
    OPEN stmt_cur;
    LOOP
      FETCH stmt_cur INTO sql_id, sql_text;
      EXIT WHEN stmt_cur%NOTFOUND;
 
      force_matching_signature := 
        dbms_sqltune_internal.i_sqltext_to_signature(sql_text, 1);

      EXECUTE IMMEDIATE
        'UPDATE wri$_sqlset_statements '                                      ||
         '  SET force_matching_signature = :1 WHERE sql_id = :2 '
      USING force_matching_signature, sql_id;
...
DECLARE
  ...
  tab_w_errors  exception;
  pragma        exception_init(tab_w_errors, -4063);

BEGIN
  FOR buf_rec IN buf_cur LOOP
    BEGIN
      alt_stmt1 := 'ALTER TABLE ' ||
                   buf_rec.schema || '.' || 'AQ$_' || buf_rec.name || '_P' ||
                   ' DROP PRIMARY KEY';
    ...
    EXCEPTION
      WHEN tab_w_errors THEN
        dbms_system.ksdwrt(dbms_system.alert_file, 
           'upgrade of queue table:'||buf_rec.schema||'.'||buf_rec.name||
           ' to 10.2 failed when altering constraints. '||
           ' Queue table had errors, recreate the queue table');
    END;

    -- No automatic stats collection for AQ spill tables
    DBMS_STATS.UNLOCK_TABLE_STATS(buf_rec.schema, 'AQ$_'||buf_rec.name||'_P');
    DBMS_STATS.DELETE_TABLE_STATS(buf_rec.schema, 'AQ$_'||buf_rec.name||'_P');
...
DECLARE
  ...
  SELECT qt.schema, qt.name, qt.flags
    FROM system.aq$_queue_tables qt;
  qt_w_errors   exception;
  pragma        exception_init(qt_w_errors, -24203);
  tab_w_errors  exception;
  pragma        exception_init(tab_w_errors, -4063);
...
BEGIN
  ...
  FOR qt_rec IN qt_cur LOOP
    BEGIN        
      ...
      END IF;
    EXCEPTION
      WHEN qt_w_errors OR tab_w_errors THEN
        dbms_system.ksdwrt(dbms_system.alert_file, 
           'upgrade of queue table:'||qt_rec.schema||'.'||qt_rec.name||
           ' to 10.2 failed when recreating views.'||
           ' Queue table had errors, recreate the queue table');
    END;

    -- No automatic stats collection for AQ tables
    DBMS_STATS.UNLOCK_TABLE_STATS(qt_rec.schema, qt_rec.name);
    DBMS_STATS.DELETE_TABLE_STATS(qt_rec.schema, qt_rec.name);
...
BEGIN
  FOR r_ret in c_ret LOOP
    BEGIN
      c := 'ALTER TABLE "' || r_ret.owner || '"."' || r_ret.table_name || '"';
      c := c || ' MODIFY LOB("' || r_ret.column_name || '") (retention)';  
      EXECUTE IMMEDIATE c;
    END;
  END LOOP;
END;
