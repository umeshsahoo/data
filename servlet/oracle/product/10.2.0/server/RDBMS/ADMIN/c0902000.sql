Rem
Rem $Header: c0902000.sql 02-may-2005.13:37:08 mtakahar Exp $
Rem
Rem c0902000.sql
Rem
...
declare
  ...
  SELECT o.obj#,
    CASE
      WHEN u.name = 'PUBLIC'
        THEN 'ALTER PUBLIC SYNONYM "' || o.name || '" COMPILE'
      ELSE
       'ALTER SYNONYM' || ' "' || u.name || '"."' || o.name || '" '
        || 'COMPILE '
      END
...
/* populate  ntimestamp# column */
UPDATE fga_log$
    SET  ntimestamp# = SYS_EXTRACT_UTC (
...
BEGIN
  ...
  -- construct Alter Table statement and execute it
  sql_stmt := 'ALTER TABLE ' || schema_name || '.AUD$ MODIFY (' ||
                ' timestamp#           DATE  NULL'     ||
                ')';
  EXECUTE IMMEDIATE sql_stmt;
END;
...
EXCEPTION
   WHEN OTHERS THEN
      IF SQLCODE IN (-942, -1917, -1918, -1919, -1951, -1952) THEN NULL;
      ELSE RAISE;
      END IF;
END;
/
