Rem
Rem $Header: e1001000.sql 22-jun-2005.11:36:27 dsampath Exp $
Rem
Rem e1001000.sql
Rem
...
Rem    dsampath    06/22/05 - bug 4449955 downgrade for sqltext signature 
...
BEGIN
  ...
  LOOP  
    ...
    new_sig := sys.dbms_sqltune_internal.i_sqltext_to_signature(sqltext, 
                                                                flag);
    ...
    SELECT a.task_id,a.name
      ...
      WHERE a.task_id = b.id
        and not exists (select c.name from sys.wri$_adv_def_parameters c
                        where c.advisor_id = b.advisor_id
                          and c.name = a.name);
...
-- Plan types
DROP TYPE sql_plan_table_type
/
DROP PUBLIC SYNONYM sql_plan_table_type
/
DROP TYPE sql_plan_row_type
/
...
BEGIN
  ...
  execute immediate 'alter table system.logmnr_log$ add constraint
    ...
    USING INDEX TABLESPACE ' || tableSpaceName || ' LOGGING';
END;
/
...
BEGIN
  ...
  FOR qt_rec IN qt_cur LOOP
   ...
   sys.dbms_aqadm_sys.execute_stmt('DROP VIEW ' || '"'||qt_rec.schema||'"."' ||
                   'AQ$_' || qt_rec.name|| '_F"', TRUE);
 END LOOP;
...
BEGIN
  FOR qt_rec IN qt_cur LOOP
    BEGIN
      dv_stmt := 'DROP VIEW ' || '"' || qt_rec.schema || '"' || '.' ||
                 '"' ||'AQ$' || qt_rec.name|| '_D' || '"';
...
truncate table dbsnmp.mgmt_bsln_intervals;
truncate table dbsnmp.mgmt_bsln_metrics;
truncate table dbsnmp.mgmt_bsln_statistics;
truncate table dbsnmp.mgmt_bsln_threshold_parms;
truncate table dbsnmp.mgmt_bsln_baselines;
truncate table dbsnmp.mgmt_bsln_datasources;
truncate table dbsnmp.mgmt_bsln_rawdata;
...
Rem put all downgrade actions above this pl/sql block
