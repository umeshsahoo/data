Rem
Rem $Header: catsqlt.sql 11-may-2005.15:45:03 pbelknap Exp $
Rem
Rem catsqlt.sql
Rem
...
--
-- PRIMARY KEY:
--     (task_id, object_id, attribute, id)
--
-- FOREIGN KEY:
...
--
-- FOREIGN KEY:
--     - (task_id, object_id, plan_attr, operation_id) 
--       references   wri$_adv_sqlt_plans(task_id, object_id, attribute, id) 
--     - (rtn_id, task_id) 
...
(
  ...
  last_load_time  DATE,
  constraint wri$_sqlset_plans_tocap primary key (stmt_id, plan_hash_value)
)
/
...
--     in the SQL tuning set. This table has the exact schema as the plan_table.
--
-- PRIMARY KEY:
...
--
-- NOTE:
--     DO NOT DOCUMENT THIS TABLE. IT IS FOR INTERNL USE ONLY!!!!!!
--     
--------------------------------------------------------------------------------
...
NESTED TABLE sql_plan STORE AS wri$_sqlset_workspace_cplans
tablespace SYSAUX
/

-- FIXME: we should create an index on nested_table_id but I do not want to do so
-- now because of bug#4356157
