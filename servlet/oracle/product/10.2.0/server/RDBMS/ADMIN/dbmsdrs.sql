...
create or replace PACKAGE dbms_drs IS
  ...
  -- And processed, it is added to a request queue and a request id is returned.
  -- Therefore, the only reason why the non-blocking functions would raise 
  -- an exeption is when the request cannot be added to the request queue.
  -- 
  -- The user may periodically check the V$DRS_REQUESTS_V view to see whether 
  -- the request has finished. If it has finished, then there will be one or
  -- more pieces of the outgoing document in the 'outdoc' column of the 
  -- V$DRS_REQUESTS_V view. Note that the request has not finished until the
  -- first piece (piece=1) of the outgoing document is NOT NULL. Pieces 
  -- are listed in order starting from 1. Each piece is up to 4000 bytes
  -- in either UTF8 or UTF16. The outgoing document types are described in
  -- the DR Server API design specification.
  --
  -- 
  ...
  PROCEDURE GetMiv(oMiv OUT BINARY_INTEGER,
                   oVersion OUT BINARY_INTEGER);

pragma TIMESTAMP('2001-08-20:12:00:00');
