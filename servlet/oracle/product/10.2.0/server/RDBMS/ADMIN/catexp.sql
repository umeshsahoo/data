Rem
Rem $Header: catexp.sql 11-may-2005.12:48:51 cdilling Exp $ expvew.sql
Rem
Rem Copyright (c) 1987, 2005, Oracle. All rights reserved.  
Rem NAME
...
CREATE OR REPLACE VIEW imp9con (
                ctname, tbname, username, intcol, type) AS
...
CREATE OR REPLACE VIEW exu9pds (
                ...
                pcache, blocksize, hscompress, defsubpcnt) AS
        ...
        WHERE   s$.obj# = o$.obj# AND
                ...
                NVL(s$.owner, 'SYS') NOT IN
                   ('ORDSYS', 'MDSYS', 'CTXSYS', 'ORDPLUGINS', 'LBACSYS',
                    'XDB', 'SI_INFORMTN_SCHEMA', 'DIP', 'DMSYS', 
		    'DBSNMP', 'EXFSYS','WMSYS') AND
                NOT EXISTS (
                    SELECT obj#
                    FROM sys.obj$ ne
...
CREATE OR REPLACE VIEW exu9sto (
                ...
                tsname, isonline, blocksize, hscompress) AS
...
CREATE OR REPLACE VIEW exu10snap (
        ...
        SELECT  s.sowner, u.user#, s.vname, s.tname, s.mview, s.mlink, s.mtime,
                ...
                */
                ...
                8, 0, s.alias_txt, c.comment$, s.syn_count
...
CREATE OR REPLACE VIEW exu81assoc (
                ...
                statsname, selectivity, cpu_cost, io_cost, net_cost) AS
...
CREATE OR REPLACE VIEW exu9plb (
                ...
                blocksize, intcolid, opaquetype) AS
...
CREATE OR REPLACE VIEW exu9typ (
        ...
        WHERE   o.type# = 13 AND
                ...
                u.name NOT IN ('ORDSYS', 'MDSYS', 'CTXSYS', 'ORDPLUGINS',
                               'LBACSYS', 'XDB', 'SI_INFORMTN_SCHEMA',
                               'DIP', 'DMSYS', 'DBSNMP', 'EXFSYS', 'WMSYS') AND
                o.spare1 = sv.version# (+)
/
