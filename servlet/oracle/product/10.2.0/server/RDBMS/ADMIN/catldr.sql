...
Rem
Rem View which returns READ/WRITE permission on an oracle directory
Rem object for the querying user.
Rem NOTE:
Rem  First case:
Rem    SYS owns all directory objects, hence has read/write privilege
Rem    on all directory objects.  Users with CREATE/DROP ANY DIRECTORY
Rem    privilege (-177, -178, respectively) have read/write privilege
Rem    on all directory objects.
Rem  Second case:
Rem    Usage of "group by" to group all directory objects
Rem    for which the requesting user has a read(17)/write(18) privilege
Rem    grant.  The sum(decode) results in either non-zero, or zero if
Rem    the requesting user has a corresponding grant.
Rem
...
Rem  own UID.
Rem
create or replace view LOADER_DIR_OBJS (name, path, read, write) as
...
(TABLE_OWNER, TABLE_NAME, COLNAME, PROPERTY, ISENCRYPTED)
as
