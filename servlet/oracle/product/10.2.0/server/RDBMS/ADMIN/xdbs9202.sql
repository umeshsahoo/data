Rem
Rem $Header: xdbs9202.sql 13-apr-2005.15:33:38 vkapoor Exp $
Rem
Rem xdbs9202.sql
Rem
...
where op.obj# in 
   (select o.obj#
    from obj$ o, user$ u, xdb.xdb$element e
...
begin
  ...
  select n into m from xdb.migr9202status for update;
  if (m < 412) then
    execute immediate 'alter type xdb.xdb$facet_list_t reset';
    execute immediate 'update obj$ set status = 6 where name = '||
                      '''XDB$FACET_LIST_T'' and owner# = '||
                      '(select user# from user$ where name = ''XDB'')';
    execute immediate 'commit';
    execute immediate 'alter system flush shared_pool';
    execute immediate 'alter type xdb.xdb$facet_list_t compile';
    execute immediate 'alter table XDB.XDB$SIMPLE_TYPE upgrade';
    execute immediate 'update xdb.migr9202status set n = 412';
    execute immediate 'alter table XDB.XDB$COMPLEX_TYPE upgrade';
    execute immediate 'commit';
  end if;
end;
/
...
begin
  ...
  if (m < 470) then
    ...
    if inst_schm_map then
      create_type_str := 'create or replace type "' || owner_var_sc || '".' ||
  ...
  if (m < 472) then
    ...
    if inst_schm_map then
      type_name := '"' || owner_var_sc || '"."schemaLocation-mapping-25_T"';
      attr1     := 'SYS_XDBPD$ XDB.XDB$RAW_LIST_T';
      attr2_nm  := '"schemaLocation-mapping"';
      attr2_ty  := '"' || owner_var_sc || '"."schemaLocation-mappi27_COLL"';
  ...
  if (m < 475) then
    ...
    if inst_schm_map then
      create_type_str := 'create or replace type "' || owner_var_sc || '".' ||
                         ...
                         'SYS_XDBPD$ xdb.XDB$RAW_LIST_T,'                   ||
                         ' "extension" "' ||owner_var_sc||'"."extension29_COLL")';
  ...
  dbms_sql.parse(
    cur, 
    'update xdb.xdb$config c set c.' || colname_xmb || ' = 1024, ' ||
    'c.' || colname_lus || ' = 16',
    dbms_sql.native);
  rc := dbms_sql.execute(cur);
  dbms_sql.close_cursor(cur);
