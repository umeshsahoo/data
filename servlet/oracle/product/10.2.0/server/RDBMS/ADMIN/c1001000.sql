Rem
Rem $Header: c1001000.sql 02-sep-2005.01:47:22 rburns Exp $
Rem
Rem c1001000.sql
Rem
...
Rem    rburns      08/18/05 - add signature to registry
Rem    rburns      08/03/05 - add edition to registry 
...
insert into TABLE_PRIVILEGE_MAP values (28, 'MERGE');

Rem===========================================================================
Rem Grant CREATE EXTERNAL JOB to all users with CREATE JOB (for compatibility)
Rem==========================================================================
DECLARE
  TYPE varchartab IS TABLE OF VARCHAR2(60);
...
BEGIN
  ...
  LOOP
    EXECUTE IMMEDIATE 'GRANT CREATE EXTERNAL JOB TO ' || user_clauses(i);
  END LOOP;
END;
/
...
BEGIN
  EXECUTE IMMEDIATE 'REVOKE EXECUTE ON AggXMLImp FROM PUBLIC';
EXCEPTION
  WHEN OTHERS THEN
    IF SQLCODE IN ( -04042, -1927 ) THEN NULL;
    ELSE RAISE;
...
BEGIN
  EXECUTE IMMEDIATE 'REVOKE EXECUTE ON XMLSeq_Imp_t FROM PUBLIC';
EXCEPTION
  WHEN OTHERS THEN
    IF SQLCODE IN ( -04042, -1927 ) THEN NULL;
    ELSE RAISE;
...
BEGIN
  EXECUTE IMMEDIATE 'REVOKE EXECUTE ON XMLSeqCur_Imp_t FROM PUBLIC';
EXCEPTION
  WHEN OTHERS THEN
    IF SQLCODE IN ( -04042, -1927 ) THEN NULL;
    ELSE RAISE;
...
BEGIN
  EXECUTE IMMEDIATE 'REVOKE EXECUTE ON XMLSeqCur2_Imp_t FROM PUBLIC';
EXCEPTION
  WHEN OTHERS THEN
    IF SQLCODE IN ( -04042, -1927 ) THEN NULL;
    ELSE RAISE;
...
exception
 ...
 if (ind_name is not null) then
   execute immediate 'drop index ' || ind_owner || '.' || ind_name;
 end if;
end;
/
...
Rem turn server output off in case catproc script leaves it on
set serveroutput off

Rem=========================================================================
Rem END STAGE 2: run the current release version of catalog and catproc
Rem=========================================================================

Rem=========================================================================
Rem BEGIN STAGE 3: Complete upgrade steps
Rem=========================================================================
...
Rem Upgrade each of the components loaded into the database

@@cmpdbmig

Rem=========================================================================
Rem END STAGE 3: Complete upgrade steps
Rem=========================================================================

Rem When updating this script for the next release, move steps 2 and 3 to
Rem the next release script and uncomment the line below.
