Rem
Rem $Header: catspace.sql 06-may-2005.15:26:38 weizhang Exp $
Rem
Rem catspace.sql
Rem
...
as
  ...
  group by ktfbfetsn
union all
  select ktfbuesegtsn, count(*), sum(ktfbueblks)
    from sys.x$ktfbue e, sys.recyclebin$ rb, sys.ts$ ts
...
/
create or replace view TBS_SPACE_USAGE(tsn, owner, bytes, blocks)
as select user_ts.tsn, user_ts.uname, nvl(sum(bytes), 0), nvl(sum(blocks), 0)
from dba_segments, user_ts
where user_ts.tsname = dba_segments.tablespace_name (+)
and   user_ts.uname = dba_segments.owner(+)
group by user_ts.tsn, user_ts.uname
/
create or replace view USER_TS_QUOTAS
    (TABLESPACE_NAME, BYTES, MAX_BYTES, BLOCKS, MAX_BLOCKS, DROPPED)
as
select ts.name, nvl(spc.bytes,0),
       decode(q.maxblocks, -1, -1, q.maxblocks * ts.blocksize),
       nvl(spc.blocks,0), q.maxblocks, decode(ts.online$, 3, 'YES', 'NO')
from sys.tsq$ q, sys.ts$ ts, sys.tbs_space_usage spc, sys.user$ usr
where q.ts# = ts.ts#
  and q.user# = userenv('SCHEMAID')
  and usr.user# = q.user#
  and spc.tsn = ts.ts#
  and spc.owner = usr.name 
/
...
select ts.name, u.name, nvl(spc.bytes, 0),
       decode(q.maxblocks, -1, -1, q.maxblocks * ts.blocksize),
       nvl(spc.blocks, 0), q.maxblocks, decode(ts.online$, 3, 'YES', 'NO')
from sys.tsq$ q, sys.ts$ ts, sys.tbs_space_usage spc, sys.user$ u
where q.ts# = ts.ts#
  and q.user# = u.user#
  and spc.tsn  = ts.ts#
