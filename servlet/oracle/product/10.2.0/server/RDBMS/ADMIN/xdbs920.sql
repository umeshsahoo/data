Rem
Rem $Header: xdbs920.sql 24-jun-2003.18:58:25 spannala Exp $
Rem
Rem xdbs920.sql
Rem
...
-- Add the 'resource-view-cache-size' element to the 'sysconfig' object type
 ...
 if (m < 300) then
  ...
  dbms_sql.parse(
    cur, 
    'alter type "' || owner_var_sc || '"."' || type_var_sc ||
...
-- Add the 'buffer-size' element to the 'ftpconfig' object type
 ...
 if (m < 330) then
  ...
  dbms_sql.parse(
    cur, 
    'alter type "' || owner_var_fc || '"."' || type_var_fc ||
...
-- Add the 'default-url-charset' element to the 'httpconfig' object type
 ...
 if (m < 340) then
  ...
  dbms_sql.parse(
    cur, 
    'alter type "' || owner_var_hc || '"."' || type_var_hc ||
...
-- Now populate the new columns in the XDB$CONFIG table
 ...
 select c.name into colname_bs from col$ c, obj$ o, attrcol$ a where
  ...
  dbms_sql.parse(
    cur, 
    'update xdb.xdb$config c set c.' || colname_rvcs || ' = 1048576, ' ||
    'c.' || colname_acs || ' = 32, c.' || colname_bs || ' = 8192',
    dbms_sql.native);
  rc := dbms_sql.execute(cur);
  dbms_sql.close_cursor(cur);
